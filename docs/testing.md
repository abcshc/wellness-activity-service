...
같은 테스트에는 48시간을 넘는 활동, KST 자정 경계 활동, 0초 구간을 함께 넣는 시나리오도 있습니다. 이 시나리오는 원본 이벤트 수와 일별 집계 행 수를 먼저 확인한 뒤, Daily와 Monthly API 결과가 날짜별 기여값의 독립 계산 결과와 일치하는지 검증합니다. 따라서 자정 경계 배분이 저장 시 한 번만 반영되고, 조회 결과가 일별 집계와 일관되는지를 확인합니다.
