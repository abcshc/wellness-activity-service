...
`ActivityUploadBatchStorageContractIntegrationTest`는 저장 구현을 개선해도 바뀌면 안 되는 결과를 MySQL에서 고정합니다. 이 테스트는 현재의 저장 방식을 성능 측정하는 테스트가 아닙니다. 이후 저장 방식을 청크 단위 배치로 바꿔도, 아래 결과가 유지되는지 확인하는 안전망입니다.
...
101·201건도 포함하는 이유는 이후 청크 크기를 도입할 때 청크 경계 바로 다음 항목이 누락되거나 중복되지 않는지 확인하기 위해서입니다.
