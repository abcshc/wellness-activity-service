...
현재 Daily·Monthly 조회는 원본 활동 이벤트를 직접 합산해 계산합니다. 다만 대용량 조회 전환을 위해 KST 일별 집계 테이블의 스키마와 영속 매핑을 추가했으며, 원본 저장 성공분을 집계에 반영하고 조회를 전환하는 작업은 아직 구현하지 않았습니다. Redis 캐시는 사용하지 않습니다.
...
`recordkey`와 원본 이벤트 식별값은 데이터베이스 유니크 제약조건으로 보호합니다. 업로드 서비스는 최대 200개 항목씩 JDBC prepared batch의 `INSERT IGNORE`를 실행해 신규 저장과 동시 재전송을 같은 흐름으로 처리하며, 이미 존재하는 이벤트는 오류 없이 무시합니다. 항목별 저장 결과에서 실제 신규 원본만 식별하므로, 이후 일별 집계 갱신에도 같은 결과를 사용할 수 있습니다. 이 방식은 여러 애플리케이션 인스턴스에서도 동일하게 동작합니다.
