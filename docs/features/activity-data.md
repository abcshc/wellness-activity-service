...
현재 Daily·Monthly 조회는 원본 활동 이벤트를 직접 합산해 계산합니다. 업로드 시에는 실제 신규 원본 이벤트만 KST 일별 집계 테이블에 같은 트랜잭션으로 반영합니다. 날짜별 집계 행은 MySQL UPSERT로 원자적으로 누적하며, 집계 갱신이 실패하면 원본 이벤트 저장도 함께 rollback됩니다. 조회를 일별 집계 테이블로 전환하는 작업은 아직 진행하지 않았습니다. Redis 캐시는 사용하지 않습니다.
