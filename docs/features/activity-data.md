...
Daily 조회는 KST 일별 집계 테이블을 읽고, 행이 없는 날짜를 `0`값으로 채웁니다. Monthly 조회는 같은 일별 집계 행을 월 단위로 합산합니다. 업로드 시에는 실제 신규 원본 이벤트만 KST 일별 집계 테이블에 같은 트랜잭션으로 반영합니다. 날짜별 집계 행은 MySQL UPSERT로 원자적으로 누적하며, 집계 갱신이 실패하면 원본 이벤트 저장도 함께 rollback됩니다. Redis 캐시는 사용하지 않습니다.
...
`recordkey`와 원본 이벤트 식별값은 데이터베이스 유니크 제약조건으로 보호합니다. 업로드 서비스는 최대 200개 항목씩 JDBC prepared batch의 `INSERT IGNORE`를 실행해 신규 저장과 동시 재전송을 같은 흐름으로 처리하며, 이미 존재하는 이벤트는 오류 없이 무시합니다. 항목별 저장 결과에서 실제 신규 원본만 식별해 그 항목의 KST 일별 기여값만 집계에 더합니다. 따라서 같은 JSON을 재전송해도 원본·일별 집계가 다시 증가하지 않습니다. 이 방식은 여러 애플리케이션 인스턴스에서도 동일하게 동작합니다.
